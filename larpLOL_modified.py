...
# ─── JUDGMENT VARIATION ───────────────────────────────────────────────────────
# Funky Friday Sick window = ~50ms. To land a "Good" we delay the tap press
# by 60-110ms — just past Sick, safely inside Good. Hold notes are NEVER
# delayed because a delayed hold press risks missing the note entirely.
GOOD_CHANCE    = 0.30    # 30% of tap notes will be intentionally "Good"
GOOD_DELAY_MIN = 0.060   # 60ms  — just outside the ~50ms Sick window
GOOD_DELAY_MAX = 0.110   # 110ms — well inside Good, safely away from OK/Bad
                ...
                if state == 'idle' and now - hold_released_at[i] >= HOLD_RELEASE_COOLDOWN:
                    ...
                    if white_count >= MIN_PIXELS:
                        ...
                        if is_hold:
                            ...
                            # ── Good variation: delay 30% of taps into the Good window ──
                            if random.random() < GOOD_CHANCE:
                                good_delay = random.uniform(GOOD_DELAY_MIN, GOOD_DELAY_MAX)
