...
# ─── JUDGMENT VARIATION ───────────────────────────────────────────────────────────────
# 30% of tap notes are intentionally delayed into the Good window.
# Uses a NON-BLOCKING deferred press — no sleep() anywhere.
# The main loop keeps running every frame so dense burst songs (GHOST etc.)
# never get misses from the macro being frozen during a delay.
GOOD_CHANCE    = 0.30   # 30% Good, 70% Sick
GOOD_DELAY_MIN = 0.055  # 55ms  — just past the Sick window (~50ms)
GOOD_DELAY_MAX = 0.075  # 75ms  — tight band, well inside Good, nowhere near OK/Bad
...
def run(status_var=None, stop_event=None):
    ...
    # ── Deferred Good press system ────────────────────────────────────────────
    # pending_press[i] = timestamp to fire the tap press (0.0 = no pending press)
    # This lets us delay 30% of taps into the Good window WITHOUT blocking the
    # main loop — critical for dense songs like GHOST where sleep() causes misses.
    pending_press    = [0.0]    * 4
    ...
    def release_all():
        for i in range(4):
            ...
            pending_press[i] = 0.0   # cancel any deferred Good presses
        flush_miss_log()
    ...
    with mss.mss() as sct:
        while True:
            ...
            # ── Fire deferred Good presses when their time arrives ────────────────
            for i in range(4):
                if pending_press[i] > 0 and now >= pending_press[i]:
                    key_press(KEYS[i])
                ...
                if state == 'idle' and now - hold_released_at[i] >= HOLD_RELEASE_COOLDOWN:
                    ...
                    if white_count >= MIN_PIXELS:
                        ...
                        if is_hold:
                            ...
                            # ── Good variation: 30% of taps are deferred (non-blocking) ──
                            # pending_press schedules the actual key_press for later.
                            # The loop keeps running so dense notes (like GHOST) are never missed.
                            if random.random() < GOOD_CHANCE:
                                delay = random.uniform(GOOD_DELAY_MIN, GOOD_DELAY_MAX)
                                pending_press[i]    = now + delay
                                states[i]           = 'pending'  # lock lane so we don't re-trigger every frame
                                note_was_pressed[i] = True  # count as pressed so no miss logged
                            else:
                                key_press(key)
                                tap_release_at[i]   = now + TAP_KEY_DURATION
                                states[i]           = 'tapped'
